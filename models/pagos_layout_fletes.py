...
class PagosLayout(models.Model):
    ...
    relacion_pagos_fletes = fields.One2many('account.payment','relacion_layout_fletes',size=64,tracking=True,track_visibility='always',store=True)
    ...
    presupuestos_rel = fields.Many2one(string='Presupuesto', comodel_name='presupuesto_flete', store=True)

    total_layout = fields.Float(string='Total Layout', compute='total_calculate')

    relacion_facturas = fields.One2many('account.move', 'pagos_layout_rel_fletes', string='Relacion Facturas')
    ...
    def add_invoices_to_list(self):
        ...
        for recx in self:
            linesx = []
            for linez in recx.presupuestos_rel.fletes_provisionados:
                print(linez.relacion_fletes_factutas.id)
                if (linez.relacion_fletes_factutas.payment_state == 'not_paid' or linez.relacion_fletes_factutas.payment_state == 'partial'):
                    linesx.append((4, int(linez.relacion_fletes_factutas.id)))
                else:
                    print('nothing data')
        ...
        print(linesx)
        recx.relacion_facturas = linesx


    def export_txt_layout_santander_flete(self):
        print('santander txt')
        for record in self:
    ...
    def export_txt_layout_other_banks(self):
        ...
        for record in self:
                ...
                for line in record.relacion_pagos_fletes:
    ...
    def total_calculate(self):
        total_lotes = 0.0
        for rec in self.relacion_pagos_fletes:
...
(6, 0, [IDs])          replace the list of linked IDs (like using (5) then (4,ID) for each ID in the list of IDs)
    ...
    def action_register_payment(self):
        ...
        return {
            ...
            'context': {
                ...
                'active_ids': self.relacion_facturas.ids,
            },
            'target': 'new',
    ...
    def pruebas(self):
        ...
        for rec in self:
            lines = []
            for line in r.relacion_facturas:
                print('xxxxxxx')
                lines.append(line.id)
                print('testing data')
                print(line.id)
            tup_invoice_ids = tuple(lines)
            print(tup_invoice_ids,' Esta es una tupla')
        ...
        rec.relacion_pagos_fletes = lines




    def delete_edit_validate(self):
        invoices_from_payment = self.relacion_pagos_fletes
    ...
    def cancelar_layout(self):
        self.state = 'borrador'
        self._cr.execute('update pagos_layout_fletes set state=%s where id=%s', ('borrador', self.id))

        for xline in self.relacion_pagos_fletes:
            print(xline.id)
    ...
    def write(self, vals):
        #permite cambiar el stado
        if 'state' in vals:
            return super().write(vals)
        #no permite editar en estado de validado
