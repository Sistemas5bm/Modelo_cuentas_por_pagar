...
class CfdiRelacionados(Base):
    __tablename__ = 'CfdiRelacionados'#revisada
...
class PagosDocRel(Base):
    __tablename__ = 'View_Pagos'#revisado
    guid_document = Column('GuidDocument',String(500),primary_key=True)
...
class Conceptos(Base):
    __tablename__ = 'Conceptos'#d55e7c6d-cd0d-431c-861f-a720c819bfda
    guid_document = Column('GuidDocument', String(500), primary_key=True)
...
class CfdisContpaqiData(Base):
    ...
    __tablename__ = 'Comprobante_impuestos_view'
    ...
    tipo_relacion_desc = Column('TipoRelacionDesc', String(500))
    impuesto_iva = Column('ImporteTotalIVATraslado', Float)
    impuesto_isrr_retenido = Column('ImporteTotalISRRetenido', Float)
    impuesto_ieps_traslado = Column('ImporteTotalIEPSTraslado', Float)
    impuesto_iva_retenido = Column('ImporteTotalIVARetenido', Float)
    impuesto_traslado = Column('TotalImpuestosTrasladados', Float)
    impuesto_retenciones = Column('TotalImpuestosRetenidos', Float)
    isr_trasladado = Column('ISRTrasladado', Float)
    ieps_retenido = Column('IEPSRetenido', Float)
...
class FacturaCfdi(models.Model):
    ...
    id_pagos_db = fields.Char(string='Pagos2')
    ...
    presupuesto_flete_fac_adic_rel = fields.Many2one('presupuesto_flete', string='Presupuesto flete Rel')
    ...
    status_rep = fields.Char(string='Estatus REP', compute='get_sum_imp_pagado')

    corte_rel = fields.One2many('cortes', 'account_move_cortes_rel', string='Relacion cortes')

    rel_fltes_ = fields.One2many('fletes_modelo_tts', 'relacion_fletes_factutas', string='Relacion fletes')

    pagos_layout_rel = fields.Many2one('pagos_layout', string='Relacion Layout')

    pagos_layout_rel_fletes = fields.Many2one('pagos_layout_fletes')

    rel_purchase = fields.Many2one('purchase.order')

    sum_total_fletes = fields.Float(string='Total Fletes', compute='suma_fletes',)

    #IMPUESTOS
    impuesto_iva = fields.Float(string='Impuesto IVA')

    impuesto_isrr_retenido = fields.Float(string='ISRR Retenido')

    impuesto_ieps_traslado = fields.Float(string='IEPS Traslado')

    impuesto_iva_retenido = fields.Float(string='IVA Retenido')

    impuesto_traslado = fields.Float(string='Impuesto traslado')

    impuesto_retenciones = fields.Float(string='Retenciones')

    isr_trasladado = fields.Float(string='ISR Traslados')

    ieps_retenido = fields.Float(string='IEPS Retenido')

    conceptos = fields.Char(string='Conceptos')

    diferencia_total = fields.Float(string='Impuesto')

    def suma_fletes(self):
        for rec in self:
            rec.sum_total_fletes = 0.0
            suma = 0.0
            for sum in rec.rel_fltes_:
                suma = suma + sum.importe_mas_retencion
            rec.sum_total_fletes = suma
    ...
    def acount_paym_ivoice(self):
       for r in self:
        ...
        r.recn = x_date
        r.id_pagos = payments_ids
        r.id_pagos_x = x_date2
        r.id_pagos_db = pagos_ids_new

    # Se agrego la funcion de calculo combinada con el requiered del lote
    ...
    def download_data(self):
        ...
        filter(CfdisContpaqiData.rfc_receptor == 'BAM170904DM5').filter(CfdisContpaqiData.forma_de_pago_desc!='Aplicación de anticipos').\
            ...
            .filter(CfdisContpaqiData.tipo_comprobante != 'E')\
            .all()
        print('FACTURAZ VIEW')
        print(cfdis_objeto)
        ...
        for record in cfdis_objeto:
            ...
            if self.env['account.move'].search_count([('uuid', '=', record.uuid)]) == 0:
             ...
             for line in contactos_obj.search([('vat', '=', record.rfc_emisor)]):
                ...
                if record.impuesto_iva > 0:
                    conceptos = conceptos+ 'IVA, '
                if record.impuesto_isrr_retenido > 0:
                    conceptos= conceptos+'ISRR Retenido, '
                if record.impuesto_ieps_traslado >0:
                    conceptos =conceptos+'IEPS traslado, '
                if record.impuesto_iva_retenido > 0:
                    conceptos= conceptos+'IVA Retenido, '
                if record.impuesto_traslado > 0:
                    conceptos= conceptos+'Traslado, '
                if record.impuesto_retenciones > 0:
                    conceptos= conceptos+'Retenciones, '
                if record.isr_trasladado > 0:
                    conceptos= conceptos+'ISR Traslado, '
                if record.ieps_retenido > 0:
                    conceptos = conceptos+'IEPS Retenidos'

                suma = record.total_impuestos_traslado - record.total_impuestos_retenidos
                ...
                recordObject = {'uuid': record.uuid,
                                ...
                                'impuesto_iva': record.impuesto_iva,
                                'impuesto_isrr_retenido': record.impuesto_isrr_retenido,
                                'impuesto_ieps_traslado': record.impuesto_ieps_traslado,
                                'impuesto_iva_retenido': record.impuesto_iva_retenido,
                                'impuesto_traslado': record.impuesto_traslado,
                                'impuesto_retenciones': record.impuesto_retenciones,
                                'isr_trasladado': record.isr_trasladado,
                                'ieps_retenido': record.ieps_retenido,
                                'diferencia_total': suma,
                                'conceptos': conceptos,
                                }
